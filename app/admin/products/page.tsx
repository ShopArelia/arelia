...
export default async function AdminProductsPage() {
  ...
  const { data: products } = await supabase
    .from("products")
    .select("id, title, price, ngo_id, created_at, ngos(name)")
    ...
    <div className="min-h-screen bg-surface-50">
      <header className="h-[60px] bg-white border-b border-surface-100 flex items-center justify-between px-8 sticky top-0 z-50">
        <div className="flex items-center gap-3">
          ...
          <span className="text-[13px] font-medium text-surface-400">Products</span>
        </div>
        <Link
      ...
      <main className="max-w-[860px] mx-auto px-6 py-12">
        <h1 className="font-serif text-[28px] font-normal tracking-[-0.015em] text-surface-400 mb-2">Products</h1>
        <p className="text-[13px] text-surface-300 mb-10">{products?.length ?? 0} products total.</p>
 
        <div className="flex flex-col divide-y divide-surface-100">
          {products?.length === 0 && (
            <p className="text-[14px] text-surface-300 py-12 text-center">
              No products yet.{" "}
              <Link href="/admin/products/new" className="text-primary-300 underline underline-offset-2">Add your first one →</Link>
            </p>
          )}
          {products?.map((p) => (
            <Link
            ...
            >
              ...
              <div className="shrink-0 flex items-center gap-4">
                <span className="text-[13px] font-medium text-surface-400">${p.price}</span>
                <span className="text-[12px] text-surface-200 group-hover:text-primary-300 transition-colors flex items-center gap-1">
                  Edit
